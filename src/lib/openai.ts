...
export async function analyzeImage(base64Image: string): Promise<AnalysisResult> {
  try {
          ...
          content: [
            {
              ...
              Responde ÚNICAMENTE con el JSON en formato plano, sin marcadores Markdown, sin triple backticks y sin texto adicional.`
            },
            {
              type: "image_url",
              image_url: {
    ...
    let jsonResponse;
    try {
      // Limpiar la respuesta para eliminar marcadores Markdown
      let responseContent = response.choices[0].message.content || '{}';
      
      // Eliminar marcadores de código Markdown (```json y ```)
...
Responde ÚNICAMENTE con un array JSON de ingredientes en formato plano, sin marcadores Markdown, sin triple backticks y sin texto adicional.`
        }
      ],
      max_tokens: 1000
    });
    ...
    try {
      ...
      // Eliminar marcadores de código Markdown (```json y ```)
      responseContent = responseContent.replace(/^```json\s*/m, '').replace(/\s*```$/m, '');
      responseContent = responseContent.replace(/^```\s*/m, '').replace(/\s*```$/m, '');
      
      const ingredients = JSON.parse(responseContent);
